...
Table~\ref{tab:disease_span} describes how the 52 diseases are distributed in our dataset. The class most prevalent in crops, unsurprisingly, is the {\em healthy} disease class at 12. Many of these diseases were aggregated across the parent datasets; one example is {\em scab} which combines {\em apple\_scab} (PlantVillage), {\em Apple\_Scab\_Leaf} (PlantDoc), {\em apple scab} (PlantWild), and {\em peach scab} (PlantWild). It must be noted that merging was done on the basis of visual-phenotype grouping and string normalization; it follows that the disease's pathogen type is not tracked here, which is, admittedly, a limitation of our dataset. Meanwhile, in terms of crop variety per disease class, 38 span exactly one crop whereas the rest exist in more than one crop. While not to the extent of the crop classes, the disease classes suffer from a long-tailed distribution as well, with {\em healthy} comprising 24\% of the dataset (15,907 images) while diseases such as {\em fire blight} only have 34 images. Further details about the full dataset shall be made available in its public release.
...
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{1.9cm} c >{\raggedright\arraybackslash}p{4.0cm} r r r r}
\hline
