...
On the subject of mobile deployment, Sharma \etal~\cite{sharma2025} utilized a combination of MobileNetV2 and ResNet50, which is a model with an optimized number of parameters while achieving over 99\% F1-score on tomato crop detection. However, this could have been expanded to other crops as well. Another such example is Liu \etal~\cite{liu2025} who proposed RicePest-YOLO, an enhanced lightweight YOLOv8-based model in order to detect rice pest images. They reported a mean average precision (mAP@0.5) of 94.3\% and mAP@0.5:0.95 of 67.3\%.
...
\begin{itemize}
    \item {\bf Input:} An image of a plant.
    \item {\bf Stage 1 Output (Crop):} One of 16 coarse labels for crops.
    \item {\bf Stage 2 Output (Disease):} One of 54 disease condition labels.
    \item {\bf XAI:} The image with disease area highlighted and analysis displayed.
\end{itemize}
